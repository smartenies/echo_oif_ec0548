...
#' for continuous bmi z-scores: Healthy Start cohort
#' 
...
#' Summarize results
temp <- tibble(exposure_period = "pregnancy",
               time_point = 0,
               outcome = "zbmi",
               group = "gis variables",
...
temp2 <- tibble(exposure_period = "pregnancy",
                time_point = 0,
                outcome = "zbmi",
                group = "gis variables",
...
#' Plot
plot_name <- paste0("QGCOMP_", "pregnancy", "_0", "_zbmi_hs.jpeg")
graphics.off()
...
#' Save the model
model_name <- "QGComp_pregnancy_0_hs.rdata"
...
df_6 <- hs_data %>%
  filter(exp_period == "pregnancy" & out_period == 6) %>%
  select(all_of(exps), all_of(covars), zbmi) %>%
  na.omit() %>%
  mutate_at(.vars = vars(all_of(scale_exp)), .funs = scale_this)
...
#' Plot
plot_name <- paste0("QGCOMP_", "pregnancy", "_6", "_zbmi_hs.jpeg")
graphics.off()
jpeg(file = here::here("figs/qgcomp_plots/", plot_name))
plot(fit.qgcomp.b_6)
dev.off()
...
#' Save the model
model_name <- "QGComp_pregnancy_6_hs.rdata"
...
df_12 <- hs_data %>%
  filter(exp_period == "pregnancy" & out_period == 12) %>%
  select(all_of(exps), all_of(covars), zbmi) %>%
  na.omit() %>%
  mutate_at(.vars = vars(all_of(scale_exp)), .funs = scale_this)
...
#' Plot
plot_name <- paste0("QGCOMP_", "pregnancy", "_12", "_zbmi_hs.jpeg")
graphics.off()
jpeg(file = here::here("figs/qgcomp_plots/", plot_name))
...
#' Save the model
model_name <- "QGComp_pregnancy_12_hs.rdata"
...
df_24 <- hs_data %>%
  filter(exp_period == "pregnancy" & out_period == 24) %>%
  select(all_of(exps), all_of(covars), zbmi) %>%
  na.omit() %>%
  mutate_at(.vars = vars(all_of(scale_exp)), .funs = scale_this)
...
#' Plot
plot_name <- paste0("QGCOMP_", "pregnancy", "_24", "_zbmi_hs.jpeg")
graphics.off()
jpeg(file = here::here("figs/qgcomp_plots/", plot_name))
...
#' Save the model
model_name <- "QGComp_pregnancy_24_hs.rdata"
...
#' Exposures
exps <- c("pm", "no2", "o3", "rhavg", "tavg_c", "bc_pred",
          "ndvi_500m", "dist_parks_m", "park_area_500m", "park_count_500m",
          "tree_cover_500m", "impervious_500m", "lila", 
          "pct_hh_poverty", "pct_less_hs_edu")
...
#' Fit the quantile g computation models adjusted for all covariates
fit.qgcomp.c_0 <- qgcomp(zbmi ~ pm + no2 + o3 + rhavg + tavg_c + bc_pred + 
                           ndvi_500m + dist_parks_m + park_area_500m + park_count_500m +
                           tree_cover_500m + impervious_500m + lila + 
                           pct_hh_poverty + pct_less_hs_edu +
                           maternal_age + maternal_partner +
                         ...
                         data = df_0, expnms = exps, family = gaussian(),
                         bayes = T)
fit.qgcomp.b_0 <- qgcomp.boot(zbmi ~ pm + no2 + o3 + rhavg + tavg_c + bc_pred + 
                                ndvi_500m + dist_parks_m + park_area_500m + park_count_500m +
                                tree_cover_500m + impervious_500m + lila + 
                                pct_hh_poverty + pct_less_hs_edu +
                                maternal_age + maternal_partner +
...
#' Plot
plot_name <- paste0("QGCOMP_", "pregnancy", "_0", "_zbmi_st_hs.jpeg")
graphics.off()
jpeg(file = here::here("figs/qgcomp_plots/", plot_name))
plot(fit.qgcomp.b_0)
dev.off()
...
#' Save the model
model_name <- "QGComp_pregnancy_0_st_hs.rdata"
save(fit.qgcomp.c_0, fit.qgcomp.b_0,
     file = here::here("results/qgcomp_results", model_name))
...
#' Fit the quantile g computation models adjusted for all covariates
fit.qgcomp.c_6 <- qgcomp(zbmi ~ pm + no2 + o3 + rhavg + tavg_c + bc_pred + 
                           ndvi_500m + dist_parks_m + park_area_500m + park_count_500m +
                           tree_cover_500m + impervious_500m + lila + 
                           pct_hh_poverty + pct_less_hs_edu +
                           maternal_age + maternal_partner +
                         ...
                         data = df_6, expnms = exps, family = gaussian(),
                         bayes = T)
fit.qgcomp.b_6 <- qgcomp.boot(zbmi ~ pm + no2 + o3 + rhavg + tavg_c + bc_pred + 
                                ndvi_500m + dist_parks_m + park_area_500m + park_count_500m +
                                tree_cover_500m + impervious_500m + lila + 
                                pct_hh_poverty + pct_less_hs_edu +
                                maternal_age + maternal_partner +
...
#' Plot
plot_name <- paste0("QGCOMP_", "pregnancy", "_6", "_zbmi_st_hs.jpeg")
graphics.off()
jpeg(file = here::here("figs/qgcomp_plots/", plot_name))
plot(fit.qgcomp.b_6)
dev.off()
...
#' Save the model
model_name <- "QGComp_pregnancy_6_st_hs.rdata"
save(fit.qgcomp.c_6, fit.qgcomp.b_6,
     file = here::here("results/qgcomp_results", model_name))
...
#' Fit the quantile g computation models adjusted for all covariates
fit.qgcomp.c_12 <- qgcomp(zbmi ~ pm + no2 + o3 + rhavg + tavg_c + bc_pred + 
                            ndvi_500m + dist_parks_m + park_area_500m + park_count_500m +
                            tree_cover_500m + impervious_500m + lila + 
                            pct_hh_poverty + pct_less_hs_edu +
                            maternal_age + maternal_partner +
                          ...
                          data = df_12, expnms = exps, family = gaussian(),
                          bayes = T)
fit.qgcomp.b_12 <- qgcomp.boot(zbmi ~ pm + no2 + o3 + rhavg + tavg_c + bc_pred + 
                                 ndvi_500m + dist_parks_m + park_area_500m + park_count_500m +
                                 tree_cover_500m + impervious_500m + lila + 
                                 pct_hh_poverty + pct_less_hs_edu +
                                 maternal_age + maternal_partner +
...
#' Plot
plot_name <- paste0("QGCOMP_", "pregnancy", "_12", "_zbmi_st_hs.jpeg")
graphics.off()
jpeg(file = here::here("figs/qgcomp_plots/", plot_name))
plot(fit.qgcomp.b_12)
dev.off()
...
#' Save the model
model_name <- "QGComp_pregnancy_12_st_hs.rdata"
save(fit.qgcomp.c_12, fit.qgcomp.b_12,
     file = here::here("results/qgcomp_results", model_name))
...
#' Fit the quantile g computation models adjusted for all covariates
fit.qgcomp.c_24 <- qgcomp(zbmi ~ pm + no2 + o3 + rhavg + tavg_c + bc_pred + 
                            ndvi_500m + dist_parks_m + park_area_500m + park_count_500m +
                            tree_cover_500m + impervious_500m + lila + 
                            pct_hh_poverty + pct_less_hs_edu +
                            maternal_age + maternal_partner +
                          ...
                          data = df_24, expnms = exps, family = gaussian(),
                          bayes = T)
fit.qgcomp.b_24 <- qgcomp.boot(zbmi ~ pm + no2 + o3 + rhavg + tavg_c + bc_pred + 
                                 ndvi_500m + dist_parks_m + park_area_500m + park_count_500m +
                                 tree_cover_500m + impervious_500m + lila + 
                                 pct_hh_poverty + pct_less_hs_edu +
                                 maternal_age + maternal_partner +
...
#' Plot
plot_name <- paste0("QGCOMP_", "pregnancy", "_24", "_zbmi_st_hs.jpeg")
graphics.off()
jpeg(file = here::here("figs/qgcomp_plots/", plot_name))
plot(fit.qgcomp.b_24)
dev.off()
...
#' Save the model
model_name <- "QGComp_pregnancy_24_st_hs.rdata"
save(fit.qgcomp.c_24, fit.qgcomp.b_24,
     file = here::here("results/qgcomp_results", model_name))
...
write_csv(gcomp_psi_results, file = here::here("results/qgcomp_results/psi_est", "qgcomp_pregnancy_psi_estimates_hs.csv"))
write_csv(gcomp_weights_results, file = here::here("results/qgcomp_results/weights", "qgcomp_pregnancy_weights_hs.csv"))